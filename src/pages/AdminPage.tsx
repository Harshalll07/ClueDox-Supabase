...
const AdminPage = () => {
  ...
  const toggleAiProvider = async (useGemini: boolean) => {
    // Permanent Gemini for standalone version
    toast.info("This system is configured to use Direct Gemini API exclusively.");
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { icon: HardDrive, label: "Total Files", value: Object.values(fileStats).reduce((s, f) => s + (f as UserFileStats).file_count, 0), color: "text-info", bg: "bg-info/10" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Status: Connected to Direct Gemini API</p>
            </div>
          </div>
          <div className="flex items-center gap-4 pl-12 text-sm text-muted-foreground">
            <Sparkles className="w-4 h-4" />
            <span>AI analysis is powered by your Google Gemini API key.</span>
          </div>
        </motion.div>
