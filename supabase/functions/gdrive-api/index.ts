...
function isValidRedirectUri(uri: string) {
  return uri.startsWith("https://") || uri.startsWith("http://localhost");
}

// ✅ AUTH URL (NO AUTH REQUIRED)
async function handleAuthUrl(body: any) {
  const clientId = Deno.env.get("GOOGLE_CLIENT_ID");

  if (!clientId) {
    return jsonResponse({ error: "GOOGLE_CLIENT_ID missing" }, 500);
  }
  ...
  try {
    ...
    if (action === "exchange-code") {
      ...
      if (!clientId || !clientSecret) {
        return jsonResponse({ error: "Google credentials missing" }, 500);
      }
...
if (!tokenRow) {
  return jsonResponse({ error: "Drive not connected" }, 403);
}

const driveAccessToken = tokenRow.access_token;
  ...
  if (!resp.ok) {
    ...
    return jsonResponse({ error: "Failed to fetch Drive files" }, 500);
  }
